...
class APIResourcesTestCase(APITestCase):
    ...
    def test_bucketlists_resource_requires_auth(self):
        post_response = self.client.post('/bucketlists/', {'name': 'test bucketlist'})
    ...
    def test_crud_operations_on_bucketlist_when_autheticated(self):
        self.client.credentials(HTTP_AUTHORIZATION='JWT ' + self.token)
        response = self.client.post('/bucketlists/', {'name': 'test bucketlist'})
        ...
        single_bucketlist = self.client.get('/bucketlists/' + str(bucketlist_id) + '/')
        ...
        updated_bucketlist = self.client.put('/bucketlists/' + str(bucketlist_id) + '/', {'name': 'new bucketlist'})
        ...
        another_item = self.client.post('/bucketlists/' + str(bucketlist_id) + '/items/', {'name': 'another item', 'parent_bucketlist': bucketlist_id})
        delete_bucketlist = self.client.delete('/bucketlists/' + str(bucketlist_id) + '/')
        bucketlist_item = self.client.get('/bucketlists/' + str(bucketlist_id) + '/items/' + str(another_item.data['id']) + '/')
    ...
    def test_crud_operations_on_item_in_bucketlist_when_authenticated(self):
        self.client.credentials(HTTP_AUTHORIZATION='JWT ' + self.token)
        another_bucketlist = self.client.post('/bucketlists/', {'name': 'another test bucketlist'})
        bucketlist_id = another_bucketlist.data['id']

        # Creating item in bucketlist
        response = self.client.post('/bucketlists/' + str(bucketlist_id) + '/items/', {'name': 'test item', 'parent_bucketlist': bucketlist_id})
        ...
        created_item = self.client.get('/bucketlists/' + str(bucketlist_id) + '/items/' + str(item_id) + '/')
        ...
        # Test updating item
        updated_item = self.client.put('/bucketlists/' + str(bucketlist_id) + '/items/' + str(item_id) + '/', {'name': 'new item', 'parent_bucketlist': bucketlist_id})
