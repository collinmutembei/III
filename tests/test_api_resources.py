...
class APIResourcesTestCase(APITestCase):
    def setUp(self):
        ...
        token_response = self.client.post(
            "/auth/login/", {"username": "tester", "password": "test"}
        )
    ...
    def test_bucketlists_api_endpoint(self):
        ...
        self.assertEqual(
            response.data["bucketlists"], "http://testserver/api/bucketlists/"
        )

    def test_bucketlists_resource_requires_auth(self):
        post_response = self.client.post(
            "/api/bucketlists/", {"name": "test bucketlist"}
        )
    ...
    def test_crud_operations_on_bucketlist_when_autheticated(self):
        ...
        single_bucketlist = self.client.get(
            "/api/bucketlists/" + str(bucketlist_id) + "/"
        )
        ...
        updated_bucketlist = self.client.put(
            "/api/bucketlists/" + str(bucketlist_id) + "/", {"name": "new bucketlist"}
        )
        ...
        another_item = self.client.post(
            "/api/bucketlists/" + str(bucketlist_id) + "/items/",
            {"name": "another item", "parent_bucketlist": bucketlist_id},
        )
        delete_bucketlist = self.client.delete(
            "/api/bucketlists/" + str(bucketlist_id) + "/"
        )
        bucketlist_item = self.client.get(
            "/api/bucketlists/"
            + str(bucketlist_id)
            + "/items/"
            + str(another_item.data["id"])
            + "/"
        )
    ...
    def test_crud_operations_on_item_in_bucketlist_when_authenticated(self):
        self.client.credentials(HTTP_AUTHORIZATION="JWT " + self.token)
        another_bucketlist = self.client.post(
            "/api/bucketlists/", {"name": "another test bucketlist"}
        )
        ...
        response = self.client.post(
            "/api/bucketlists/" + str(bucketlist_id) + "/items/",
            {"name": "test item", "parent_bucketlist": bucketlist_id},
        )
        ...
        created_item = self.client.get(
            "/api/bucketlists/" + str(bucketlist_id) + "/items/" + str(item_id) + "/"
        )
        ...
        updated_item = self.client.put(
            "/api/bucketlists/" + str(bucketlist_id) + "/items/" + str(item_id) + "/",
            {"name": "new item", "parent_bucketlist": bucketlist_id},
        )
